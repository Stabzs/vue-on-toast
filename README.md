...
| showCloseButton | `boolean|object` | false | By default, the close button is not shown for any toast type.  This can be overridden for all toast types by providing `false`, or it can be overridden per toast type as an object (`showCloseButton: {error: true}}`). |
